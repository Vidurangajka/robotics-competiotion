...
//Maze Variables
int junction = 0;
int junc_count = 0;  //For junction zero  count is 1
int box_indicator = 5;
...
void loop() {
  //Barcode readind
  if (flag == 0) {  //Moving out from White box
    while (true) {
      sensorRead();
  ...
  Serial.println("Out from white");
  if (n == NUM_LINES + 1) {
    goForward(10);
  } else {
    goForward(30);
    if (u < 1) {
      Serial.println("Barcode Reading");
      barcode();
  ...
  Serial.println("last line barcode");
  if (n == NUM_LINES) {
    goForward(700);
  } else {
    goForward(30);
  ...
  switch (junction) {
    ...
    case 1:
      while (true) {
        ...
        if (obstacle == "F") {
          if (junc_count == 5 || junc_count == 6 || junc_count == 7) {  //Reverse stabilize by PID
            PID_control_Reverse();
          ...
          if (junc_count == 15 || junc_count == 19 || junc_count == 20) {
            PID_control_Reverse();
          } else if (junc_count == 22) {
            digitalWrite(5, LOW);
            stop();
            delay(1000);

          } else if (junc_count < 20) {
            PID_control_Forward();
          }
        }
        ...
        if (dVal[0] == 1 && dVal[1] == 1 && dVal[2] == 1 && dVal[3] == 1 && dVal[4] == 1 && dVal[5] == 1 && dVal[6] == 1 && dVal[7] == 1) {  //Left line detect
          ...
          Serial.println(junc_count);
          if (junc_count == 5 && obstacle == "F" || junc_count == 6 && obstacle == "F") {
            digitalWrite(box_indicator, HIGH);
          } else if (junc_count == 13 && obstacle == "T" || junc_count == 14 && obstacle == "T" || junc_count == 19 && obstacle == "T" || junc_count == 20 && obstacle == "T"|| junc_count == 21 && obstacle == "T") {
            digitalWrite(box_indicator, HIGH);
          } else {
            digitalWrite(box_indicator, LOW);
          }
    ...
    case 2:
      while (true) {
        ...
        if (dVal[0] == 1 && dVal[1] == 1 && dVal[2] == 1 && dVal[3] == 1 && dVal[4] == 1 && dVal[5] == 1 && dVal[6] == 1 && dVal[7] == 1) {  //Left line detect
          junc_count = junc_count + 1;
          Serial.println(junc_count);
          if (junc_count == 13 || junc_count == 14) {
            digitalWrite(box_indicator, HIGH);
...
void decode(float timeArray[NUM_LINES]) {
  ...
  junction = binaryToDecimal(decodeArray, 8);
  junction = junction % 5;
...
void Zero_Junction_common() {
  ...
  if (junc_count == 1 || junc_count == 4 || junc_count == 16) {  //Right turn
    if (junc_count == 16 || junc_count == 1) {
      goForward(goforwardDelay);
    } else {
      goForward(550);
    }
    unsigned long startTime = millis();
...
void Zero_Junction_Case2() {
  ...
  if (junc_count == 18 || junc_count == 19 || junc_count == 23) {  //Right turn
    if (junc_count == 18 || junc_count == 19) {
      goForward(goforwardDelay);
    } else {
      goForward(550);
    }
...
void First_Junction_Case1() {
  if (junc_count == 4) {  //Right turn
    ...
    Right_normal();
    PID_control_Forward();
  } else if (junc_count == 6) {  //Reverse junc_count == 5 ||
    goBackword(300);
  }
}
void First_Junction_Case2() {
  ...
  if (junc_count == 6 || junc_count == 7 || junc_count == 8 || junc_count == 17 || junc_count == 18) {  //Right turn
    if (junc_count == 6) {
      goForward(500);
    } else if (junc_count == 7 || junc_count == 17) {
      goForward(1070);
    } else {
      goForward(500);
    }
...
void Second_Junction_common() {
  ...
  if (junc_count == 1 == 1 || junc_count == 3) {  //Right turn
    if (junc_count == 1) {
      goForward(1200);
    } else {
      goForward(goforwardDelay);
